  ...
  adminLoginForm.addEventListener('submit', (e) => { e.preventDefault(); const u = document.getElementById('admin-username').value; const p = document.getElementById('admin-password').value; if (u === "namseljaya" && p === "4dmin69#") { hideAdminLoginModal(); formContainer.classList.add('hidden'); adminContainer.classList.remove('hidden'); tabAdmin.classList.add('bg-indigo-600','text-white', 'tab-active'); tabAdmin.classList.remove('hover:bg-gray-50', 'text-gray-600'); tabForm.classList.remove('bg-indigo-600','text-white', 'tab-active'); tabForm.classList.add('hover:bg-gray-50', 'text-gray-600'); startAdminPanel(); } else { Swal.fire({ icon: 'error', title: 'Login Gagal', text: 'Username atau password salah!' }); } });
